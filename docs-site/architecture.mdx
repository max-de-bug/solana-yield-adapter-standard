...
<Note>
In this session the folder structure was consolidated: `adapters/` symlinks removed, all source moved to `programs/`, `Cargo.toml` workspace members updated to `programs/` paths, and an **adapter-template** scaffold was added.
</Note>
...
Program ID (devnet): `8u4YFQiTCR5n5dijVoinXyZ962ngVmFuWKELDUjVCqAR`
...
Program ID (devnet): `8TAhAne1z4chGzuP9EeXFuYsqyGHzACWuD7sURS3ydAq`
...
Six Anchor programs (five reference implementations + one template scaffold). Each implements the same three-instruction surface with protocol-specific PDA seeds and optional constraints (e.g. Drift two-phase withdrawal).

| Adapter | Devnet program ID | Notable trait |
|---|---|---|
| Kamino | `BQMHrbTGx9ruKQN54XzMajLq769ax3e33YJ5FMkowrg9` | Golden reference — simplest full implementation |
| MarginFi | `LtccLreoDVj2vurvsWpvfC8PvYTnUpTaxz6P9pDg5Y2` | Standard share vault |
| Jupiter | `8QdkGAkLvpN7JPxf3dgKFUXVGPS2LWW4BumbNkVkXkux` | JLP-themed naming |
| Maple | `GRyFctNGZFhHnpHFyyB8xtYdVtC58ZuwyC63PrEy3Vrk` | Real syrupUSDC (yield-bearing SPL) |
| Drift | `2zMNZcFzAx9bFNchTWDqiJGt5H3bCDgo8PW1TTskwcLJ` | Two-phase withdrawal (13d cooldown) |
| Template | `jbLUHXvc9P26MpQdGXht4aKnbn68i2GijxsFX6RXahV` | No-op scaffold for building new adapters |
